        ...
        private void Start()
{
    ...
    _fallTimeoutDelta = FallTimeout;
        ...
        private void Move()
        {
            ...
            // NEW BEHAVIOR: Horizontal input (A/D) rotates the player in place
            // Vertical input (W/S) moves the player forward/backward relative to player's facing direction
            if (_input.move != Vector2.zero)
            {
                // Rotate player based on horizontal input (A/D)
                if (Mathf.Abs(_input.move.x) > 0.1f)
                {
                    float rotationAmount = _input.move.x * RotationSmoothTime * 10f;
                    ...
                    // Also rotate the camera target to match player's new facing direction
                    CinemachineCameraTarget.transform.rotation = Quaternion.Euler(
                        _cinemachineTargetPitch + CameraAngleOverride,
                        _targetRotation,
                        0.0f
                ...
                // Move player forward/backward based on vertical input (W/S)
